...
function mockRequest(): Request {
  return new Request("http://localhost/api/reports/csv");
}
...
describe("GET /api/reports/csv", () => {
  ...
  it("includes business transactions and the user's own personal transactions", async () => {
    ...
    expect(body).toContain("business-lunch");
    expect(body).toContain("my-coffee");
    expect(prisma.transaction.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: {
          ...
          OR: [
            { scope: "BUSINESS" },
            { scope: "PERSONAL", userId: fakeProfile.id },
          ],
        },
      })
    );
  ...
  it("excludes another member's personal transactions", async () => {
    ...
    expect(body).toContain("business-lunch");
    expect(body).toContain("my-coffee");
    expect(body).not.toContain("other-member-secret");
  });
